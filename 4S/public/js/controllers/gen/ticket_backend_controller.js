...
app.controller('TicketBackendController', ['$scope', '$http', '$upload', 'createDialog', '$log', function ($scope, $http, $upload, createDialogService, $log) {
    ...
    $scope.gridOptions.columnDefs = [        
        ...
        {field: 'name', displayName: objFieldInfo.Ticket.name, headerTooltip: '点击表头可进行排序', enableCellEdit: true},
        {field: 'status', displayName: objFieldInfo.Ticket.status, width: 120, headerTooltip: '点击表头可进行排序, 通过直接下拉选取操作来更新数据', enableCellEdit: false, cellTemplate: dropdownTemplateTicketStatus},
        {field: 'description', displayName: objFieldInfo.Ticket.description, headerTooltip: '点击表头可进行排序', enableCellEdit: true},
        {field: 'images', displayName: objFieldInfo.Ticket.images, width: 170, enableCellEdit: false, cellTemplate: uploadImageTemplateTicket},
        {field: 'lastUpdateTime', displayName: objFieldInfo.Ticket.lastUpdateTime, headerTooltip: '点击表头可进行排序', enableCellEdit: true},
        {field: 'createdAt', displayName: objFieldInfo.Ticket.createdAt, headerTooltip: '点击表头可进行排序', enableCellEdit: true},
        {field: 'comment', displayName: objFieldInfo.Ticket.comment, headerTooltip: '点击表头可进行排序', enableCellEdit: true},
        {field: 'refUserId', displayName: objFieldInfo.Ticket.refUserId, headerTooltip: '点击表头可进行排序', enableCellEdit: false, cellTemplate: '<a href="/admin/user?relate=id&relateValue={{COL_FIELD}}"><span ng-bind="COL_FIELD"></span></a>'},
        {field: 'refStaffId', displayName: objFieldInfo.Ticket.refStaffId, headerTooltip: '点击表头可进行排序', enableCellEdit: false, cellTemplate: '<a href="/admin/staff?relate=id&relateValue={{COL_FIELD}}"><span ng-bind="COL_FIELD"></span></a>'},

        {field: 'user', displayName: objFieldInfo.Ticket.user, headerTooltip: '点击表头可进行排序', enableCellEdit: false, cellTemplate: '<a href="/admin/user?relate=id&relateValue={{COL_FIELD.id}}"><span ng-bind="COL_FIELD.name"></span></a>'},
        {field: 'staff', displayName: objFieldInfo.Ticket.staff, headerTooltip: '点击表头可进行排序', enableCellEdit: false, cellTemplate: '<a href="/admin/staff?relate=id&relateValue={{COL_FIELD.id}}"><span ng-bind="COL_FIELD.name"></span></a>'},
    ];
    ...
    $scope.selectedParentStaffId = 0
    $scope.parentStaffs = []
    $scope.parentStaffs4grid = []
    $scope.pageInfo4ParentStaff = {}
    $scope.searchFieldNameStaff = searchFieldNameStaff
    $scope.searchFieldNameStaffComment = searchFieldNameStaffComment
    

    $scope.$watch('selectedParentStaffId', function(newValue, oldValue, scope){
        if(newValue != oldValue) {
            if($scope.parentStaffs.length > 0) {
                if ($scope.selectedParentStaffId) {
                    if ($scope.paginationConf.currentPage != 1) {
                        $scope.paginationConf.currentPage = 1
                    }
                } else {
    ...
    $http.get('/base/Staff/all').success(function (data, status, headers, config) {
    	if (data.flag) {
            $scope.parentStaffs = [{"id": 0, "name": "全部"}]
    		$scope.parentStaffs = $scope.parentStaffs.concat(data.data);
            if ((GetQueryString('relate') == 'staff.id' || GetQueryString('relate') == 'refStaffId') 
                && GetQueryString('relateValue')) 
                $scope.selectedParentStaffId = parseInt(GetQueryString('relateValue'))
    	}
    });
    
    function fillGridWithParentStaffs() {
        ...
        $http.get(url)
            .success(function (data, status, headers, config) {
            if (data.flag) {
                $scope.parentStaffs4grid = data.data;
                
                for (x in $scope.parentStaffs4grid) {
                    if ($scope.parentStaffs4grid[x].id === $scope.currentObj.refStaffId) {
                        $scope.parentStaffs4grid[x].refParentStaff = true
                        break
                    }
                    else {
                        $scope.parentStaffs4grid[x].refParentStaff = false
                    }
                }
            }
            else {
    ...
    $scope.myParentStaffSelections = [];
    $scope.gridParentStaffs = {
        data: 'parentStaffs4grid',
        enableRowSelection: true,
        enableRowHeaderSelection: false,
        multiSelect: false,
        onRegisterApi: function (gridApi) {
            $scope.gridApi = gridApi;
            gridApi.selection.on.rowSelectionChanged($scope, function (rows) {
                $scope.myParentStaffSelections = gridApi.selection.getSelectedRows();
            });
        },
        isRowSelectable: function(row){
            if(row.entity.refParentStaff == true){
                row.grid.api.selection.selectRow(row.entity);
            }
        },
        columnDefs: [        
    ...
    function getQueryUrl() {
        ...
        if ($scope.selectedParentStaffId != 0) {
            fieldOnParam += 'refStaffId';
            fieldValueParam += $scope.selectedParentStaffId;
        }
        ...
        $http.get(url).success(function (data, status, headers, config) {
            if (data.flag) {
                for (x in data.data) {
                    ...
                    if (!data.data[x].staff) $scope.getRefObjName(data.data[x], 'Staff')
                }
    ...
    $scope.saveContent = function() {
        ...
        if ($scope.myParentStaffSelections.length > 0) content.refStaffId = $scope.myParentStaffSelections[0].id
        ...
        if(items.length == 0){
            ...
            if(content.id){
                bootbox.confirm("您确定要删除这个对象吗?", function(result) {
                    if(result) {
                        var deleteUrl = '/base/Ticket/' + content.id
    ...
    $scope.report = function () {
        ...
        bootbox.confirm(notifyMsg, function(result) {
            if(result) {
                location.href = '/report/ticket?' + getQueryUrl()
            }
        });
    }
    ...
    ////////// web socket msg //////////
    var wsUri = ''
    var channelId = 'ticket_backend_' + getNowFormatDate()

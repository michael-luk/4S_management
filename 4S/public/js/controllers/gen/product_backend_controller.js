...
app.controller('ProductBackendController', ['$scope', '$http', '$upload', 'createDialog', '$log', function ($scope, $http, $upload, createDialogService, $log) {
    ...
    $scope.gridOptions.columnDefs = [        
        ...
        {field: 'friendPurchase', displayName: objFieldInfo.Product.purchases, width: '80', headerTooltip: '弹窗/跳转显示', enableCellEdit: false, cellTemplate: friendButtonTemplateProductPurchase},
        {field: 'friendSale', displayName: objFieldInfo.Product.sales, width: '80', headerTooltip: '弹窗/跳转显示', enableCellEdit: false, cellTemplate: friendButtonTemplateProductSale},

    ];
    
    $scope.goToFriendPagePurchase = function(pid) { location = '/admin/purchase?relate=products.id&relateValue=' + pid }
    $scope.goToFriendPageSale = function(pid) { location = '/admin/sale?relate=products.id&relateValue=' + pid }
        ...
        $http.get(url)
            ...
            if (data.flag) {
                ...
                if ($scope.currentObj.id) {
                    ...
                    //用于比较, 全取不分页
                    $http.get('/product/' + $scope.currentObj.id + '/purchases?page=1&size=1000000')
                    .success(function (data, status, headers, config) {
                        if (data.flag){
                            for (x in allFriends) {
        ...
        $http.get(url)
            ...
            if (data.flag) {
                ...
                if ($scope.currentObj.id) {
                    ...
                    //用于比较, 全取不分页
                    $http.get('/product/' + $scope.currentObj.id + '/sales?page=1&size=1000000')
                    .success(function (data, status, headers, config) {
                        if (data.flag){
                            for (x in allFriends) {
                                allFriends[x].refFriend = false
    ...
    $scope.popFriendPurchase = function (obj) {
        if (obj) {
            ...
            createDialogService("/assets/html/child_pop_templates/product_2_purchase.html", {
                id: 'friend_purchase',
                title: '查看',
                scope: $scope,
                footerTemplate: '<div></div>'
    ...
    $scope.gridFriendSale.columnDefs = [        
        ...
        {field: 'images', displayName: objFieldInfo.Sale.images, width: 170, enableCellEdit: false, cellTemplate: readonlyImageTemplateProduct},
    ...
    $scope.popFriendSale = function (obj) {
        if (obj) {
            ...
            createDialogService("/assets/html/child_pop_templates/product_2_sale.html", {
                id: 'friend_sale',
                title: '查看',
                scope: $scope,
                footerTemplate: '<div></div>'

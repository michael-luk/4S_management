...
app.controller('TalkSkillBackendController', ['$scope', '$http', '$upload', 'createDialog', '$log', function ($scope, $http, $upload, createDialogService, $log) {
	$scope.isTalkSkill = true;
    ...
    dropdownTemplateTalkSkillStatus = '<div>' +
        '<select ng-model="MODEL_COL_FIELD" ' +
        'ng-change="grid.appScope.updateEntity(col, row)" style="padding: 2px;">'
    for (var i = 0; i < Object.keys(objEnumInfo.TalkSkill.status).length; i++) {
        $scope.status.push({"id": i, "name": objEnumInfo.TalkSkill.status[i]})
        dropdownTemplateTalkSkillStatus += '<option ng-selected="MODEL_COL_FIELD==' + i
            + '" value=' + i + '>' + objEnumInfo.TalkSkill.status[i] + '</option>'
    }
    dropdownTemplateTalkSkillStatus += '</select></div>'
    ...
    $scope.gridOptions.columnDefs = [        
        ...
        {field: 'name', displayName: objFieldInfo.TalkSkill.name, headerTooltip: '点击表头可进行排序', enableCellEdit: true},
        {field: 'answer', displayName: objFieldInfo.TalkSkill.answer, headerTooltip: '点击表头可进行排序', enableCellEdit: true},
        {field: 'status', displayName: objFieldInfo.TalkSkill.status, width: 120, headerTooltip: '点击表头可进行排序, 通过直接下拉选取操作来更新数据', enableCellEdit: false, cellTemplate: dropdownTemplateTalkSkillStatus},
        {field: 'images', displayName: objFieldInfo.TalkSkill.images, width: 170, enableCellEdit: false, cellTemplate: uploadImageTemplateTalkSkill},
        {field: 'smallImages', displayName: objFieldInfo.TalkSkill.smallImages, width: 170, enableCellEdit: false, cellTemplate: uploadImageTemplateTalkSkill},
        {field: 'lastUpdateTime', displayName: objFieldInfo.TalkSkill.lastUpdateTime, headerTooltip: '点击表头可进行排序', enableCellEdit: true},
        {field: 'createdAt', displayName: objFieldInfo.TalkSkill.createdAt, headerTooltip: '点击表头可进行排序', enableCellEdit: true},
        {field: 'description', displayName: objFieldInfo.TalkSkill.description, headerTooltip: '点击表头可进行排序', enableCellEdit: true},
        {field: 'comment', displayName: objFieldInfo.TalkSkill.comment, headerTooltip: '点击表头可进行排序', enableCellEdit: true},
    ...
    function refreshData(showMsg){
        var url = '/base/TalkSkill/all?page=' 
        ...
        if(items.length == 0){
            ...
            if(content.id){
                bootbox.confirm("您确定要删除这个对象吗?", function(result) {
                    if(result) {
                        var deleteUrl = '/base/TalkSkill/' + content.id
    ...
    $scope.addContent = function(){
        $http.get('/new/talkskill')
            .success(function (data, status, headers, config) {
                if (data.flag) {
                    $scope.currentObj = data.data;
                    
                    
                    createDialogService("/assets/html/edit_templates/talk_skill.html",{
                        id: 'editor',
                        title: '新增',
                        scope: $scope,
    ...
    $scope.report = function () {
        ...
        bootbox.confirm(notifyMsg, function(result) {
            if(result) {
                location.href = '/report/talkskill?' + getQueryUrl()
            }
        });
    }
    ...
    ////////// web socket msg //////////
    var wsUri = ''
    var channelId = 'talk_skill_backend_' + getNowFormatDate()

...
app.controller('FixBackendController', ['$scope', '$http', '$upload', 'createDialog', '$log', function ($scope, $http, $upload, createDialogService, $log) {
	$scope.isFix = true;
    ...
    dropdownTemplateFixStatus = '<div>' +
        '<select ng-model="MODEL_COL_FIELD" ' +
        'ng-change="grid.appScope.updateEntity(col, row)" style="padding: 2px;">'
    for (var i = 0; i < Object.keys(objEnumInfo.Fix.status).length; i++) {
        $scope.status.push({"id": i, "name": objEnumInfo.Fix.status[i]})
        dropdownTemplateFixStatus += '<option ng-selected="MODEL_COL_FIELD==' + i
            + '" value=' + i + '>' + objEnumInfo.Fix.status[i] + '</option>'
    }
    dropdownTemplateFixStatus += '</select></div>'
    ...
    $scope.gridOptions.columnDefs = [        
        ...
        {field: 'name', displayName: objFieldInfo.Fix.name, headerTooltip: '点击表头可进行排序', enableCellEdit: true},
        {field: 'status', displayName: objFieldInfo.Fix.status, width: 120, headerTooltip: '点击表头可进行排序, 通过直接下拉选取操作来更新数据', enableCellEdit: false, cellTemplate: dropdownTemplateFixStatus},
        {field: 'refCarId', displayName: objFieldInfo.Fix.refCarId, headerTooltip: '点击表头可进行排序', enableCellEdit: false, cellTemplate: '<a href="/admin/car?relate=id&relateValue={{COL_FIELD}}"><span ng-bind="COL_FIELD"></span></a>'},
        {field: 'images', displayName: objFieldInfo.Fix.images, width: 170, enableCellEdit: false, cellTemplate: uploadImageTemplateFix},
        {field: 'lastUpdateTime', displayName: objFieldInfo.Fix.lastUpdateTime, headerTooltip: '点击表头可进行排序', enableCellEdit: true},
        {field: 'createdAt', displayName: objFieldInfo.Fix.createdAt, headerTooltip: '点击表头可进行排序', enableCellEdit: true},
        {field: 'description1', displayName: objFieldInfo.Fix.description1, headerTooltip: '点击表头可进行排序', enableCellEdit: true},
        {field: 'description2', displayName: objFieldInfo.Fix.description2, headerTooltip: '点击表头可进行排序', enableCellEdit: true},
        {field: 'comment', displayName: objFieldInfo.Fix.comment, headerTooltip: '点击表头可进行排序', enableCellEdit: true},

        {field: 'car', displayName: objFieldInfo.Fix.car, headerTooltip: '点击表头可进行排序', enableCellEdit: false, cellTemplate: '<a href="/admin/car?relate=id&relateValue={{COL_FIELD.id}}"><span ng-bind="COL_FIELD.name"></span></a>'},
    ];
    
    
    $scope.selectedParentCarId = 0
    $scope.parentCars = []
    $scope.parentCars4grid = []
    $scope.pageInfo4ParentCar = {}
    $scope.searchFieldNameCar = searchFieldNameCar
    $scope.searchFieldNameCarComment = searchFieldNameCarComment
    

    $scope.$watch('selectedParentCarId', function(newValue, oldValue, scope){
        if(newValue != oldValue) {
            if($scope.parentCars.length > 0) {
                if ($scope.selectedParentCarId) {
                    if ($scope.paginationConf.currentPage != 1) {
                        $scope.paginationConf.currentPage = 1
                    }
                } else {
    ...
    $http.get('/base/Car/all').success(function (data, status, headers, config) {
    	if (data.flag) {
            $scope.parentCars = [{"id": 0, "name": "全部"}]
    		$scope.parentCars = $scope.parentCars.concat(data.data);
            if ((GetQueryString('relate') == 'car.id' || GetQueryString('relate') == 'refCarId') 
                && GetQueryString('relateValue')) 
                $scope.selectedParentCarId = parseInt(GetQueryString('relateValue'))
    	}
    });
    
    function fillGridWithParentCars() {
        url = '/base/Car/all?page=1' 
                    + '&size=1000000'
                    
        if ($scope.currentObj.queryKeyword4Car)
            url += '&searchOn=' + $scope.searchFieldNameCar + '&kw=' + $scope.currentObj.queryKeyword4Car
            
        $http.get(url)
            .success(function (data, status, headers, config) {
            if (data.flag) {
                $scope.parentCars4grid = data.data;
                
                for (x in $scope.parentCars4grid) {
                    if ($scope.parentCars4grid[x].id === $scope.currentObj.refCarId) {
                        $scope.parentCars4grid[x].refParentCar = true
                        break
                    }
                    else {
                        $scope.parentCars4grid[x].refParentCar = false
                    }
                }
            }
            else {
    ...
    $scope.myParentCarSelections = [];
    $scope.gridParentCars = {
        data: 'parentCars4grid',
        enableRowSelection: true,
        enableRowHeaderSelection: false,
        multiSelect: false,
        onRegisterApi: function (gridApi) {
            $scope.gridApi = gridApi;
            gridApi.selection.on.rowSelectionChanged($scope, function (rows) {
                $scope.myParentCarSelections = gridApi.selection.getSelectedRows();
            });
        },
        isRowSelectable: function(row){
            if(row.entity.refParentCar == true){
    ...
    $scope.searchContent4Car = function(){
        fillGridWithParentCars()
    }
    ...
    function getQueryUrl() {
        ...
        if ($scope.selectedParentCarId != 0) {
            fieldOnParam += 'refCarId';
            fieldValueParam += $scope.selectedParentCarId;
        }
    ...
    function refreshData(showMsg){
        ...
        $http.get(url).success(function (data, status, headers, config) {
            if (data.flag) {
                for (x in data.data) {
                    if (!data.data[x].car) $scope.getRefObjName(data.data[x], 'Car')
                }
    ...
    $scope.saveContent = function() {
        ...
        if ($scope.myParentCarSelections.length > 0) content.refCarId = $scope.myParentCarSelections[0].id
        ...
        if(items.length == 0){
            ...
            if(content.id){
                bootbox.confirm("您确定要删除这个对象吗?", function(result) {
                    if(result) {
                        var deleteUrl = '/base/Fix/' + content.id
        ...
        $http.get('/new/fix')
            .success(function (data, status, headers, config) {
                if (data.flag) {
                    ...
                    createDialogService("/assets/html/edit_templates/fix.html",{
                        id: 'editor',
                        title: '新增',
                        scope: $scope,
    ...
    $scope.report = function () {
        ...
        bootbox.confirm(notifyMsg, function(result) {
            if(result) {
                location.href = '/report/fix?' + getQueryUrl()
            }
        });
    }
    ...
    ////////// web socket msg //////////
    var wsUri = ''
    var channelId = 'fix_backend_' + getNowFormatDate()

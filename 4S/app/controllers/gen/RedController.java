...
public class RedController extends Controller implements IConst {
    
    public static Result redPage(Integer status, Integer notStatus,
                                     String fieldOn, String fieldValue, boolean isAnd,
                                     String searchOn, String kw,
                                     String startTime, String endTime,
                                     String order, String sort,
                                     Integer page, Integer size) {
        Msg<List<Red>> msg = BaseController.doGetAll("Red",
                status, notStatus,
    ...
    public static Result redBackendPage() {
        return ok(red_backend.render());
    }
    
    public static Result getRedUsers(Long refId, Integer page, Integer size) {
        if (size == 0)
            size = PAGE_SIZE;
        if (page <= 0)
            page = 1;
        ...
        Red found = Red.find.byId(refId);
        if (found != null) {
            if (found.users.size() > 0) {
                Page<User> records;
                records = User.find.where().eq("reds.id", refId).orderBy("id desc").findPagingList(size)
            ...
            msg.message = NO_FOUND;
            play.Logger.info("red result: " + NO_FOUND);
        }
    ...
    public static Result add() {
        ...
        Form<RedParser> httpForm = form(RedParser.class).bindFromRequest();
        if (!httpForm.hasErrors()) {
            RedParser formObj = httpForm.get();            
            Red newObj = new Red();
            
            String uniqueFieldIssue = BaseController.checkFieldUnique("Red", formObj);
            if (StrUtil.isNotNull(uniqueFieldIssue)) {
                msg.message = "字段[" + TableInfoReader.getFieldComment("Red", uniqueFieldIssue)
                        + "]存在同名数据";
                return ok(Json.toJson(msg));
            }
            ...
            try{
                ...
                sendWebSocketByChannelTag("red_backend", "new");
    ...
    public static Result update(long id) {
        ...
        if (!httpForm.hasErrors()) {
            ...
            String uniqueFieldIssue = BaseController.checkFieldUnique("Red", formObj, 1);
            if (StrUtil.isNotNull(uniqueFieldIssue)) {
                msg.message = "字段[" + TableInfoReader.getFieldComment("Red", uniqueFieldIssue)
                        + "]存在同名数据";
                return ok(Json.toJson(msg));
            ...
            try{
                ...
                // 处理多对多 red <-> User, 先清掉对面的
                for (User refObj : found.users) {
                    if (refObj.reds.contains(found)) {
                        refObj.reds.remove(found);
                        refObj.save();
                    }
                }
                    ...
                    for (User jsonRefObj : formObj.users) {
                        for (User dbRefObj : allRefUsers) {
                            if (dbRefObj.id == jsonRefObj.id) {
                                ...
                                if (!dbRefObj.reds.contains(found)) {
                                    dbRefObj.reds.add(found);
                                    dbRefObj.save();
                                }
                                break;
                            }
    ...
    public static class RedParser {
        ...
        public String validate() {
            if (StrUtil.isNull(name)) {
                return TableInfoReader.getFieldComment(Red.class, "name") + "不能为空";
            }
    ...
    public static Result delete(long id) {
        ...
        if (found != null) {
            ...
            try{
                ...
                sendWebSocketByChannelTag("red_backend", "delete");
            } catch (PersistenceException ex){
    ...
    public static File getReportFile(String fileName,
        ...
        Msg<List<Red>> msg = BaseController.doGetAll("Red",
                ...
                startTime, endTime,
                "createdAt", "desc",
                1, Red.find.findRowCount());
		List<Red> list = msg.data;        
		...
		// 创建工作表对象，并命名
		HSSFSheet sheet2 = workbook2007.createSheet(TableInfoReader.getTableComment(Red.class) + "报表");
		// 设置列
		...
		title.createCell(0).setCellValue(TableInfoReader.getTableComment(Red.class) + "报表");
        ...
        titleRow.createCell(0).setCellValue(TableInfoReader.getFieldComment(Red.class, "name"));//name
        titleRow.createCell(1).setCellValue(TableInfoReader.getFieldComment(Red.class, "status"));//status
        titleRow.createCell(2).setCellValue(TableInfoReader.getFieldComment(Red.class, "money"));//money
        titleRow.createCell(3).setCellValue(TableInfoReader.getFieldComment(Red.class, "lastUpdateTime"));//last_update_time
        titleRow.createCell(4).setCellValue(TableInfoReader.getFieldComment(Red.class, "createdAt"));//created_at
        titleRow.createCell(5).setCellValue(TableInfoReader.getFieldComment(Red.class, "description1"));//description1
        titleRow.createCell(6).setCellValue(TableInfoReader.getFieldComment(Red.class, "description2"));//description2
        titleRow.createCell(7).setCellValue(TableInfoReader.getFieldComment(Red.class, "comment"));//comment
		...
		for (i = 0; i < list.size(); i++) {
            ...
            cell1.setCellValue(EnumInfoReader.getEnumName(Red.class, "status", item.status));
	...
	public static Result report(Integer status, Integer notStatus,
		...
		String fileName = TableInfoReader.getTableComment(Red.class) + "报表_" + DateUtil.NowString("yyyy_MM_dd_HH_mm_ss") + ".xls";

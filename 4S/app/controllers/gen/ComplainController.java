...
public class ComplainController extends Controller implements IConst {
    
    public static Result complainPage(Integer status, Integer notStatus,
                                     String fieldOn, String fieldValue, boolean isAnd,
                                     String searchOn, String kw,
                                     String startTime, String endTime,
                                     String order, String sort,
                                     Integer page, Integer size) {
        Msg<List<Complain>> msg = BaseController.doGetAll("Complain",
                status, notStatus,
        ...
        if (msg.flag) {
            return ok(complain.render(msg.data));
        } else {
    ...
    public static Result complainBackendPage() {
        return ok(complain_backend.render());
    }
    

    @Security.Authenticated(SecuredSuperAdmin.class)
    @MethodName("新增_Complain")
    @Role("create_complain")
    public static Result add() {
        Msg<Complain> msg = new Msg<>();

        Form<ComplainParser> httpForm = form(ComplainParser.class).bindFromRequest();
        if (!httpForm.hasErrors()) {
            ComplainParser formObj = httpForm.get();            
            Complain newObj = new Complain();
            
            String uniqueFieldIssue = BaseController.checkFieldUnique("Complain", formObj);
            if (StrUtil.isNotNull(uniqueFieldIssue)) {
                msg.message = "字段[" + TableInfoReader.getFieldComment("Complain", uniqueFieldIssue)
                        + "]存在同名数据";
            ...
            try{
                ...
                sendWebSocketByChannelTag("complain_backend", "new");
    ...
    @Security.Authenticated(SecuredSuperAdmin.class)
    @MethodName("修改_Complain")
    @Role("update_complain")
    public static Result update(long id) {
        Msg<Complain> msg = new Msg<>();

        Complain found = Complain.find.byId(id);
        if (found == null) {
        ...
        if (!httpForm.hasErrors()) {
            ...
            String uniqueFieldIssue = BaseController.checkFieldUnique("Complain", formObj, 1);
            if (StrUtil.isNotNull(uniqueFieldIssue)) {
                msg.message = "字段[" + TableInfoReader.getFieldComment("Complain", uniqueFieldIssue)
                        + "]存在同名数据";
                return ok(Json.toJson(msg));
    ...
    public static class ComplainParser {
        ...
        public String validate() {
            if (StrUtil.isNull(name)) {
                return TableInfoReader.getFieldComment(Complain.class, "name") + "不能为空";
            }
    ...
    public static File getReportFile(String fileName,
        ...
        Msg<List<Complain>> msg = BaseController.doGetAll("Complain",
                ...
                startTime, endTime,
                "createdAt", "desc",
                1, Complain.find.findRowCount());
		List<Complain> list = msg.data;        
		...
		// 创建工作表对象，并命名
		HSSFSheet sheet2 = workbook2007.createSheet(TableInfoReader.getTableComment(Complain.class) + "报表");
		// 设置列
		...
		title.createCell(0).setCellValue(TableInfoReader.getTableComment(Complain.class) + "报表");
        ...
        titleRow.createCell(0).setCellValue(TableInfoReader.getFieldComment(Complain.class, "name"));//name
        titleRow.createCell(1).setCellValue(TableInfoReader.getFieldComment(Complain.class, "status"));//status
        titleRow.createCell(2).setCellValue(TableInfoReader.getFieldComment(Complain.class, "description"));//description
        titleRow.createCell(3).setCellValue(TableInfoReader.getFieldComment(Complain.class, "user"));//user_id
        titleRow.createCell(4).setCellValue(TableInfoReader.getFieldComment(Complain.class, "lastUpdateTime"));//last_update_time
        titleRow.createCell(5).setCellValue(TableInfoReader.getFieldComment(Complain.class, "createdAt"));//created_at
        titleRow.createCell(6).setCellValue(TableInfoReader.getFieldComment(Complain.class, "comment"));//comment
		...
		for (i = 0; i < list.size(); i++) {
            ...
            cell1.setCellValue(EnumInfoReader.getEnumName(Complain.class, "status", item.status));
    ...
    @Security.Authenticated(SecuredAdmin.class)
    @MethodName("导出报表_Complain")
    @Role("report_complain")
	public static Result report(Integer status, Integer notStatus,
		...
		String fileName = TableInfoReader.getTableComment(Complain.class) + "报表_" + DateUtil.NowString("yyyy_MM_dd_HH_mm_ss") + ".xls";

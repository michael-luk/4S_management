...
public class FeedbackController extends Controller implements IConst {
    
    public static Result feedbackPage(Integer status, Integer notStatus,
                                     String fieldOn, String fieldValue, boolean isAnd,
                                     String searchOn, String kw,
                                     String startTime, String endTime,
                                     String order, String sort,
                                     Integer page, Integer size) {
        Msg<List<Feedback>> msg = BaseController.doGetAll("Feedback",
                status, notStatus,
        ...
        if (msg.flag) {
            return ok(feedback.render(msg.data));
        } else {
    ...
    public static Result feedbackBackendPage() {
        return ok(feedback_backend.render());
    }
    

    @Security.Authenticated(SecuredSuperAdmin.class)
    @MethodName("新增_Feedback")
    @Role("create_feedback")
    public static Result add() {
        Msg<Feedback> msg = new Msg<>();

        Form<FeedbackParser> httpForm = form(FeedbackParser.class).bindFromRequest();
        if (!httpForm.hasErrors()) {
            FeedbackParser formObj = httpForm.get();            
            Feedback newObj = new Feedback();
            
            String uniqueFieldIssue = BaseController.checkFieldUnique("Feedback", formObj);
            if (StrUtil.isNotNull(uniqueFieldIssue)) {
                msg.message = "字段[" + TableInfoReader.getFieldComment("Feedback", uniqueFieldIssue)
                        + "]存在同名数据";
            ...
            try{
                ...
                sendWebSocketByChannelTag("feedback_backend", "new");
    ...
    @Security.Authenticated(SecuredSuperAdmin.class)
    @MethodName("修改_Feedback")
    @Role("update_feedback")
    public static Result update(long id) {
        Msg<Feedback> msg = new Msg<>();

        Feedback found = Feedback.find.byId(id);
        if (found == null) {
        ...
        if (!httpForm.hasErrors()) {
            ...
            String uniqueFieldIssue = BaseController.checkFieldUnique("Feedback", formObj, 1);
            if (StrUtil.isNotNull(uniqueFieldIssue)) {
                msg.message = "字段[" + TableInfoReader.getFieldComment("Feedback", uniqueFieldIssue)
                        + "]存在同名数据";
                return ok(Json.toJson(msg));
    ...
    public static class FeedbackParser {
        ...
        public String validate() {
            if (StrUtil.isNull(name)) {
                return TableInfoReader.getFieldComment(Feedback.class, "name") + "不能为空";
            }
    ...
    public static File getReportFile(String fileName,
        ...
        Msg<List<Feedback>> msg = BaseController.doGetAll("Feedback",
                ...
                startTime, endTime,
                "createdAt", "desc",
                1, Feedback.find.findRowCount());
		List<Feedback> list = msg.data;        
		...
		// 创建工作表对象，并命名
		HSSFSheet sheet2 = workbook2007.createSheet(TableInfoReader.getTableComment(Feedback.class) + "报表");
		// 设置列
		...
		title.createCell(0).setCellValue(TableInfoReader.getTableComment(Feedback.class) + "报表");
        ...
        titleRow.createCell(0).setCellValue(TableInfoReader.getFieldComment(Feedback.class, "name"));//name
        titleRow.createCell(1).setCellValue(TableInfoReader.getFieldComment(Feedback.class, "status"));//status
        titleRow.createCell(2).setCellValue(TableInfoReader.getFieldComment(Feedback.class, "description"));//description
        titleRow.createCell(3).setCellValue(TableInfoReader.getFieldComment(Feedback.class, "user"));//user_id
        titleRow.createCell(4).setCellValue(TableInfoReader.getFieldComment(Feedback.class, "lastUpdateTime"));//last_update_time
        titleRow.createCell(5).setCellValue(TableInfoReader.getFieldComment(Feedback.class, "createdAt"));//created_at
        titleRow.createCell(6).setCellValue(TableInfoReader.getFieldComment(Feedback.class, "comment"));//comment
		...
		for (i = 0; i < list.size(); i++) {
            ...
            cell1.setCellValue(EnumInfoReader.getEnumName(Feedback.class, "status", item.status));
    ...
    @Security.Authenticated(SecuredAdmin.class)
    @MethodName("导出报表_Feedback")
    @Role("report_feedback")
	public static Result report(Integer status, Integer notStatus,
		...
		String fileName = TableInfoReader.getTableComment(Feedback.class) + "报表_" + DateUtil.NowString("yyyy_MM_dd_HH_mm_ss") + ".xls";

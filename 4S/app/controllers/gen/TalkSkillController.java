...
public class TalkSkillController extends Controller implements IConst {
    
    public static Result talkSkillPage(Integer status, Integer notStatus,
                                     String fieldOn, String fieldValue, boolean isAnd,
                                     String searchOn, String kw,
                                     String startTime, String endTime,
                                     String order, String sort,
                                     Integer page, Integer size) {
        Msg<List<TalkSkill>> msg = BaseController.doGetAll("TalkSkill",
                status, notStatus,
        ...
        if (msg.flag) {
            return ok(talk_skill.render(msg.data));
        } else {
    ...
    public static Result talkSkillBackendPage() {
        return ok(talk_skill_backend.render());
    }
    

    @Security.Authenticated(SecuredSuperAdmin.class)
    @MethodName("新增_TalkSkill")
    @Role("create_talk_skill")
    public static Result add() {
        Msg<TalkSkill> msg = new Msg<>();

        Form<TalkSkillParser> httpForm = form(TalkSkillParser.class).bindFromRequest();
        if (!httpForm.hasErrors()) {
            TalkSkillParser formObj = httpForm.get();            
            TalkSkill newObj = new TalkSkill();
            
            String uniqueFieldIssue = BaseController.checkFieldUnique("TalkSkill", formObj);
            if (StrUtil.isNotNull(uniqueFieldIssue)) {
                msg.message = "字段[" + TableInfoReader.getFieldComment("TalkSkill", uniqueFieldIssue)
                        + "]存在同名数据";
                return ok(Json.toJson(msg));
            }

            newObj.name = formObj.name;
            newObj.answer = formObj.answer;
            ...
            try{
                ...
                sendWebSocketByChannelTag("talk_skill_backend", "new");
    ...
    @Security.Authenticated(SecuredSuperAdmin.class)
    @MethodName("修改_TalkSkill")
    @Role("update_talk_skill")
    public static Result update(long id) {
        Msg<TalkSkill> msg = new Msg<>();

        TalkSkill found = TalkSkill.find.byId(id);
        if (found == null) {
        ...
        if (!httpForm.hasErrors()) {
            ...
            String uniqueFieldIssue = BaseController.checkFieldUnique("TalkSkill", formObj, 1);
            if (StrUtil.isNotNull(uniqueFieldIssue)) {
                msg.message = "字段[" + TableInfoReader.getFieldComment("TalkSkill", uniqueFieldIssue)
                        + "]存在同名数据";
                return ok(Json.toJson(msg));
            ...
            Transaction txn = Ebean.beginTransaction();
            try{
                found = TalkSkill.find.byId(id);
                            
                found.name = formObj.name;
                found.answer = formObj.answer;
    ...
    public static class TalkSkillParser {
        ...
        public String validate() {
            if (StrUtil.isNull(name)) {
                return TableInfoReader.getFieldComment(TalkSkill.class, "name") + "不能为空";
            }
    ...
    public static File getReportFile(String fileName,
        ...
        Msg<List<TalkSkill>> msg = BaseController.doGetAll("TalkSkill",
                ...
                startTime, endTime,
                "createdAt", "desc",
                1, TalkSkill.find.findRowCount());
		List<TalkSkill> list = msg.data;        
		...
		// 创建工作表对象，并命名
		HSSFSheet sheet2 = workbook2007.createSheet(TableInfoReader.getTableComment(TalkSkill.class) + "报表");
		// 设置列
        sheet2.setColumnWidth(0, 4000);
        sheet2.setDefaultColumnStyle(0, cellStyle);//name
        sheet2.setColumnWidth(1, 4000);
        sheet2.setDefaultColumnStyle(1, cellStyle);//answer
        ...
        sheet2.setDefaultColumnStyle(5, cellStyle);//description
		...
		title.createCell(0).setCellValue(TableInfoReader.getTableComment(TalkSkill.class) + "报表");
        ...
        titleRow.createCell(0).setCellValue(TableInfoReader.getFieldComment(TalkSkill.class, "name"));//name
        titleRow.createCell(1).setCellValue(TableInfoReader.getFieldComment(TalkSkill.class, "answer"));//answer
        titleRow.createCell(2).setCellValue(TableInfoReader.getFieldComment(TalkSkill.class, "status"));//status
        titleRow.createCell(3).setCellValue(TableInfoReader.getFieldComment(TalkSkill.class, "lastUpdateTime"));//last_update_time
        titleRow.createCell(4).setCellValue(TableInfoReader.getFieldComment(TalkSkill.class, "createdAt"));//created_at
        titleRow.createCell(5).setCellValue(TableInfoReader.getFieldComment(TalkSkill.class, "description"));//description
        titleRow.createCell(6).setCellValue(TableInfoReader.getFieldComment(TalkSkill.class, "comment"));//comment
		...
		for (i = 0; i < list.size(); i++) {
            ...
            if (item.answer == null) {
                ...
                cell1.setCellValue(item.answer);
            }
            HSSFCell cell2 = row.createCell(2);
            cell2.setCellValue(EnumInfoReader.getEnumName(TalkSkill.class, "status", item.status));
            ...
            if (item.description == null) {
                ...
                cell5.setCellValue(item.description);
            }
    ...
    @Security.Authenticated(SecuredAdmin.class)
    @MethodName("导出报表_TalkSkill")
    @Role("report_talk_skill")
	public static Result report(Integer status, Integer notStatus,
		...
		String fileName = TableInfoReader.getTableComment(TalkSkill.class) + "报表_" + DateUtil.NowString("yyyy_MM_dd_HH_mm_ss") + ".xls";

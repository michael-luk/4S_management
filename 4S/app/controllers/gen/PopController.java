...
public class PopController extends Controller implements IConst {
    
    public static Result popPage(Integer status, Integer notStatus,
                                     String fieldOn, String fieldValue, boolean isAnd,
                                     String searchOn, String kw,
                                     String startTime, String endTime,
                                     String order, String sort,
                                     Integer page, Integer size) {
        Msg<List<Pop>> msg = BaseController.doGetAll("Pop",
                status, notStatus,
    ...
    public static Result popBackendPage() {
        return ok(pop_backend.render());
    }
    
    public static Result getPopUsers(Long refId, Integer page, Integer size) {
        if (size == 0)
            size = PAGE_SIZE;
        if (page <= 0)
            page = 1;
        ...
        Pop found = Pop.find.byId(refId);
        if (found != null) {
            if (found.users.size() > 0) {
                Page<User> records;
                records = User.find.where().eq("pops.id", refId).orderBy("id desc").findPagingList(size)
            ...
            msg.message = NO_FOUND;
            play.Logger.info("pop result: " + NO_FOUND);
        }
    ...
    public static Result add() {
        ...
        Form<PopParser> httpForm = form(PopParser.class).bindFromRequest();
        if (!httpForm.hasErrors()) {
            PopParser formObj = httpForm.get();            
            Pop newObj = new Pop();
            
            String uniqueFieldIssue = BaseController.checkFieldUnique("Pop", formObj);
            if (StrUtil.isNotNull(uniqueFieldIssue)) {
                msg.message = "字段[" + TableInfoReader.getFieldComment("Pop", uniqueFieldIssue)
                        + "]存在同名数据";
                return ok(Json.toJson(msg));
            }
            ...
            try{
                ...
                sendWebSocketByChannelTag("pop_backend", "new");
    ...
    public static Result update(long id) {
        ...
        if (!httpForm.hasErrors()) {
            ...
            String uniqueFieldIssue = BaseController.checkFieldUnique("Pop", formObj, 1);
            if (StrUtil.isNotNull(uniqueFieldIssue)) {
                msg.message = "字段[" + TableInfoReader.getFieldComment("Pop", uniqueFieldIssue)
                        + "]存在同名数据";
                return ok(Json.toJson(msg));
            ...
            try{
                ...
                // 处理多对多 pop <-> User, 先清掉对面的
                for (User refObj : found.users) {
                    if (refObj.pops.contains(found)) {
                        refObj.pops.remove(found);
                        refObj.save();
                    }
                }
                    ...
                    for (User jsonRefObj : formObj.users) {
                        for (User dbRefObj : allRefUsers) {
                            if (dbRefObj.id == jsonRefObj.id) {
                                ...
                                if (!dbRefObj.pops.contains(found)) {
                                    dbRefObj.pops.add(found);
                                    dbRefObj.save();
                                }
                                break;
                            }
    ...
    public static class PopParser {
        ...
        public String validate() {
            if (StrUtil.isNull(name)) {
                return TableInfoReader.getFieldComment(Pop.class, "name") + "不能为空";
            }
    ...
    public static Result delete(long id) {
        ...
        if (found != null) {
            ...
            try{
                ...
                sendWebSocketByChannelTag("pop_backend", "delete");
            } catch (PersistenceException ex){
    ...
    public static File getReportFile(String fileName,
        ...
        Msg<List<Pop>> msg = BaseController.doGetAll("Pop",
                ...
                startTime, endTime,
                "createdAt", "desc",
                1, Pop.find.findRowCount());
		List<Pop> list = msg.data;        
		...
		// 创建工作表对象，并命名
		HSSFSheet sheet2 = workbook2007.createSheet(TableInfoReader.getTableComment(Pop.class) + "报表");
		// 设置列
		...
		title.createCell(0).setCellValue(TableInfoReader.getTableComment(Pop.class) + "报表");
        ...
        titleRow.createCell(0).setCellValue(TableInfoReader.getFieldComment(Pop.class, "name"));//name
        titleRow.createCell(1).setCellValue(TableInfoReader.getFieldComment(Pop.class, "classify"));//classify
        titleRow.createCell(2).setCellValue(TableInfoReader.getFieldComment(Pop.class, "status"));//status
        titleRow.createCell(3).setCellValue(TableInfoReader.getFieldComment(Pop.class, "lastUpdateTime"));//last_update_time
        titleRow.createCell(4).setCellValue(TableInfoReader.getFieldComment(Pop.class, "createdAt"));//created_at
        titleRow.createCell(5).setCellValue(TableInfoReader.getFieldComment(Pop.class, "description1"));//description1
        titleRow.createCell(6).setCellValue(TableInfoReader.getFieldComment(Pop.class, "description2"));//description2
        titleRow.createCell(7).setCellValue(TableInfoReader.getFieldComment(Pop.class, "comment"));//comment
		...
		for (i = 0; i < list.size(); i++) {
            ...
            cell2.setCellValue(EnumInfoReader.getEnumName(Pop.class, "status", item.status));
	...
	public static Result report(Integer status, Integer notStatus,
		...
		String fileName = TableInfoReader.getTableComment(Pop.class) + "报表_" + DateUtil.NowString("yyyy_MM_dd_HH_mm_ss") + ".xls";

...
public class ImageController extends Controller implements IConst {
    ...
    public static Result genBarcodeDo(String sourceStr, int width, int height, String format) {
        return ok(genBarcode(sourceStr, width, height, format));
    }

    public static String genBarcode(String sourceStr, int width, int height, String format) {
        if (StrUtil.isNull(sourceStr)) return null;
        if (StrUtil.isNull(format)) format = "PNG";
        if (width <= 0) width = 300;
        if (height <= 0) height = 300;

        BitMatrix bitMatrix = null;
        try {
            bitMatrix = new MultiFormatWriter().encode(sourceStr, BarcodeFormat.QR_CODE, width, height);
        } catch (WriterException e) {
            play.Logger.error("barcode create issue on source: " + sourceStr);
            return "";
        }
        ...
        String barcodeFilePath = Play.application().path().getPath() + "\\public\\barcode\\" + sourceStr + ".png";
        File file = new File(barcodeFilePath);
        try {
            ImageIO.write(image, format, file);
        } catch (IOException e) {
            play.Logger.error("barcode save issue on source: " + sourceStr);
            return "";
        }
        return sourceStr + ".png";
    }
}
